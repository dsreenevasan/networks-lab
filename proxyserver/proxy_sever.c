...
//			strcat(buf, "FILE NOT FOUND");
			 printf("Client running in port %d\n", atoi(argv[2]));
		        ...
		        int client_sock1, server_sock1, c1, read_size1, i1;
        		struct sockaddr_in server1, client1;
	        	char server_message1[1000], client_message1[1000];
...
//      		server.sin_family = AF_INET;
        		...
        		if(connect(server_sock1, (struct sockaddr*)&server1, sizeof(server1)) < 0){
        		...
        		if(send(server_sock1, "index.html", 1000, 0) < 0){
                		perror("send failed");
		                return 1;
        		}

	        	if(recv(server_sock1, server_message1, 1000, 0) < 0){
        	        	perror("recv failed");
	                	return 1;
		        }
			FILE *fp=fopen("index.html","ab+");
			fprintf(fp, "%s", server_message1);
        		fclose(fp);
			strcat(buf, server_message1);
		        close(server_sock1);
 //     	  return 0;
		}
